...
const createCollege = async function (req, res) {
    try {
        ...
        if (!isValid(fullName))  // --> fullName of the college should be provided in the body
            return res.status(400).send({ status: false, message: "Please enter the fullName of the college. " })
        if (!data.fullName.match(nameRegex))  // --> fullName should be provided in right format
            return res.status(400).send({ status: false, message: "fullName can't be alphanumeric. " })

        if (!isValid(logoLink))  // --> logoLink should be provided in the request body
            return res.status(400).send({ status: false, message: "Please enter the logoLink. " })
