...
const createIntern = async function (req, res) {
    try {
        ...
        if (!isValid(name))  // --> name should be provided in the body
            return res.status(400).send({ status: false, message: "Please enter the intern name. " })
        ...
        if (!emailRegex.test(email))  // --> email should be provided in right format
            return res.status(400).send({ status: false, message: "Please enter the emailId in right format. " })
        ...
        if (!mobileRegex.test(mobile))  // --> mobile number should be provided in right format
            return res.status(400).send({ status: false, message: "Enter the mobile number in valid Indian format. " })
        ...
        if (getMobile) {  // --> if that mobile number is already provided in the database
            return res.status(400).send({ status: false, message: "Mobile number is already in use, please enter a new one. " });
        }
        ...
        if (!college) return res.status(404).send({ status: false, message: "No such college found. " })  // --> if no such college exists
