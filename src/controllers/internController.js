...
const {
  ...
  emailRegex,
  mobileRegex,
} = require("../validations/validator"); // --> importing the validator module
...
const createIntern = async function (req, res) {
  try {
    ...
    if (Object.keys(data).length === 0)
      // --> if there is no input provided in the request body
      return res
        .status(400)
        .send({
          status: false,
          message: "Please enter the name, email, mobile and collegeId. ",
        });
    ...
    if (!isValid(name))
      ...
      return res
        .status(400)
        .send({ status: false, message: "Please enter the intern name. " });
    ...
    if (!emailRegex.test(email))
      ...
      return res
        ...
        .send({
          status: false,
          message: "Please enter the emailId in right format. ",
        });
    ...
    if (!isValid(mobile))
      // --> mobile number should be provided in the body
      return res
        .status(400)
        .send({ status: false, message: "Please enter the mobile number. " });
    if (!mobileRegex.test(mobile))
      // --> mobile number should be provided in right format
      return res
        .status(400)
        .send({
          status: false,
          message: "Enter the mobile number in valid Indian format. ",
        });
    ...
    if (getMobile) {
      // --> if that mobile number is already provided in the database
      return res
        .status(400)
        .send({
          status: false,
          message: "Mobile number is already in use, please enter a new one. ",
        });
    }

    if (!isValid(collegeName))
      // --> collegeName should be provided in the body
      return res
        .status(400)
    ...
    if (!college)
      return res
        .status(404)
        .send({ status: false, message: "No such college found. " }); // --> if no such college exists
